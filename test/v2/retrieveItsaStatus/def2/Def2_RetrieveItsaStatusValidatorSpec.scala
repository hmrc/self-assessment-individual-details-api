...
package v2.retrieveItsaStatus.def2

import shared.models.domain.{Nino, TaxYear}
import shared.models.errors.*
import shared.utils.UnitSpec
import v2.models.errors.{FutureYearsFormatError, HistoryFormatError}
import v2.retrieveItsaStatus.def2.model.request.Def2_RetrieveItsaStatusRequestData

class Def2_RetrieveItsaStatusValidatorSpec extends UnitSpec {
  ...
  private def validator(nino: String, taxYear: String, futureYears: Option[String], history: Option[String]) =
    new Def2_RetrieveItsaStatusValidator(nino, taxYear, futureYears, history)
    ...
    "return the parsed domain object" when {
      "passed a valid request" in {
        ...
        result shouldBe Right(
          Def2_RetrieveItsaStatusRequestData(parsedNino, parsedTaxYear, futureYears = false, history = false)
        )
      }
      ...
      "the request is valid and specifies futureYears and history" in {
        ...
        result shouldBe Right(
          Def2_RetrieveItsaStatusRequestData(parsedNino, parsedTaxYear, futureYears = true, history = true)
        )
      }
    }
