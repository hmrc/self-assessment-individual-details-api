...
class RequestHandlerSpec
  ...
  private val successValidatorForRequest = new Validator[Input.type] {
    protected def validate: Either[Seq[MtdError], Input.type] = Right(Input)
  }

  private val singleErrorValidatorForRequest = new Validator[Input.type] {
    protected def validate: Either[Seq[MtdError], Input.type] = Left(List(NinoFormatError))
  }

  trait MockValidatorFactory extends MockFactory {
    val mockRetrieveValidatorFactory: RetrieveItsaStatusValidatorFactory = mock[RetrieveItsaStatusValidatorFactory]
  }
